## QA Portfolio — Екатерина (rlowhat)

Учусь на тестировщика ПО (Manual QA). Здесь — практика тренировочных проектах: чек-листы, тест-кейсы и баг-репорты, оформленные по стандарту (шаги воспроизведения / ожидаемый результат / фактический результат / приоритет).

## О себе
- Среднее профессиональное образование, коммерция (по отраслям)
- Самостоятельно изучаю тестирование: жизненный цикл бага, приоритизация (Critical/Major/Minor), написание чек-листов, основы SQL, Postman, Jira/TestRail
- Английский: A2, в процессе изучения

## Проекты

### 1. OWASP Juice Shop — [08.07.2026]
Тренировочное веб-приложение с намеренно заложенными уязвимостями.
- [Чек-лист тестирования регистрации](https://github.com/rlowhat/QA-Portfolio/blob/main/bug-reports-OWASP-Juice-Shop)
- [Найденные баги](https://github.com/rlowhat/QA-Portfolio/blob/main/checklist-registration-OWASP-Juice-Shop)
### 2. Python-скрипт автоматически открывающий Juice Shop для проверки нескольких пунктов из чек-листа
- [Проект на Python для автоматизации](https://github.com/rlowhat/QA-Portfolio/blob/main/test-registation).
## Контакты
- Email: zaikinae628@gmail.com
- Telegram: @rlowhat
